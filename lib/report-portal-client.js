   ...
   */
    ...
    mergeLaunches() {
        ...
        if (this.isLaunchMergeRequired) {
            ...
            // Use the launch uuid to get the global launch id (long)
            // TODO Should only get the unfinished ones here
            this.getlaunchglobalids(launchUuIds).then(launchIds => {

                this.logDebug(`Found global ids: ${launchIds}`);
                this.logDebug("Start waiting for finish...");

                this.waitforlaunchfinish(launchIds).then(finishedStatus => {
                    this.logDebug(`All launches finished, merging: ${launchIds}`);
            ...
            rejectPromise("No merge required");
        }

        return promise;
    }

    getlaunchglobalids(launchUuids) {

        const promises = [];

        launchUuids.forEach((launchId) => {
            const getUrl = ["launch", launchId].join("/");
            
            const requestPromise = this.restClient.retrieve(getUrl, { headers: this.headers })
                .then((response) => {
                    this.logDebug(`Found launch: ${response.id} ${response.status}`);
                    return response.id;
                }, (error) => {
                    console.dir(error);
                });
    ...
    // Return launch details by global id
    getlaunches(launchIds) {
    ...
    waitforlaunchfinish(launchIds, resolvePromise) {

        this.logDebug(`Waiting for launches to finish: ${launchIds}`);
        
        const promise = resolvePromise || new Promise((resolve, reject) => {
            resolvePromise = resolve;
        });

       this.getlaunches(launchIds).then(launches => {

            const unfinishedLaunchIds = [];
            launches.forEach(launchItem => {
                if(typeof launchItem.endTime === 'undefined'){
                    unfinishedLaunchIds.push(launchItem.id);
                }
            });

            if(unfinishedLaunchIds.length > 0) {
                setTimeout(() => {
                    this.waitforlaunchfinish(unfinishedLaunchIds, resolvePromise);
                }, 5000);
            } else {
                this.logDebug("No unfinished launches");
                resolvePromise("All finished");
            }
