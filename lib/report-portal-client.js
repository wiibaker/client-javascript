...
class RPClient {
   ...
   */
    ...
    sendLaunchLog(launchTempId, saveLogRQ, fileObj) {
        const saveLogData = Object.assign({
            time: this.helpers.now(),
            message: '',
            level: '',
        }, saveLogRQ);
        const isLaunchLog = true;

        if (fileObj) {
            return this.sendLogWithFile(launchTempId, saveLogData, fileObj, isLaunchLog);
        }
        return this.sendLogWithoutFile(launchTempId, saveLogData, isLaunchLog);
    }
     ...
     *      time: this.helpers.now()
     * }
     * @param {boolean} isLaunchLog (optional) - send log to the launch.
     * @Returns {Object} - an object which contains a tempId and a promise
     */
    sendLogWithoutFile(itemTempId, saveLogRQ, isLaunchLog = false) {
        const itemObj = this.map[itemTempId];
        if (!itemObj) {
            return this.getRejectAnswer(itemTempId, new Error(`Item "${itemTempId}" not found`));
        }
        ...
        const requestPromise = (itemUuid, launchUuid) => {
            const url = 'log';
            const ids = isLaunchLog ? { launchUuid: itemUuid } : { itemUuid, launchUuid };

            return this.restClient.create(
                url,
                Object.assign(saveLogRQ, ids),
                { headers: this.headers },
            );
        };
     ...
     * @Returns {Object} - an object which contains a tempId and a promise
     */
    sendLogWithFile(itemTempId, saveLogRQ, fileObj, isLaunchLog = false) {
        const itemObj = this.map[itemTempId];
        if (!itemObj) {
            return this.getRejectAnswer(itemTempId, new Error(`Item "${itemTempId}" not found`));
        }
        ...
        const requestPromise = (itemUuid, launchUuid) => {
            const ids = isLaunchLog ? { launchUuid: itemUuid } : { itemUuid, launchUuid };
            this.getRequestLogWithFile(Object.assign(saveLogRQ, ids), fileObj);
        };
